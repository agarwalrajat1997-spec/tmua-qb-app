(function () {
  ...
  function csv(r=report()) {
    ...
    const rows=[['Pathway','Module','Question','Selected letter','Selected answer','Correct letter','Correct answer','Result','Time spent seconds','Flagged','Module started UTC','Module submitted UTC','Last answer change UTC']];
    r.modules.forEach(m => m.questions.forEach(q => rows.push([r.pathway,m.name,q.number,q.selectedAnswer || '',q.selectedAnswerText,q.correctAnswer,q.correctAnswerText,q.correct?'Correct':q.selectedAnswer?'Incorrect':'Unanswered',q.timeSpentSeconds,q.flagged?'Yes':'No',m.startedAt,m.submittedAt,q.lastAnswerChangeAt||''])));
